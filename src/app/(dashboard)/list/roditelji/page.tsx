...
import { Prisma, Roditelj, Ucenik } from ".prisma/client";
import Image from "next/image"
import FormeContainer from "@/app/components/FormeContainer";
import { auth } from "@clerk/nextjs/server";

type Roditelji = Roditelj & { ucenici: Ucenik[] }
...
const columns = [
    ...
    {
        header: "Ucenici", accessor: "ucenici", className: "hidden md:table-cell",
    },
    {
...
const renderRow = (item: Roditelji) => (
    <tr key={item.id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-sPurpleLight">
        ...
        <td className="hidden md:table-cell">{item.ucenici.map(ucenik => ucenik.ime).join(',')}</td>
        ...
        <td>
            <div className="flex items-center gap-2">
                {role === "admin" && (
                    <>
                        <FormeContainer table="roditelj" type="update" data={item} />
                        <FormeContainer table="roditelj" type="delete" id={item.id} />
                    </>
                )}
            </div>
        </td>
...
const Roditelji = async ({
    ...
    const query: Prisma.RoditeljWhereInput = {};
    ...
    const [data, count] = await prisma.$transaction([
        ...
        prisma.roditelj.count({ where: query }),
    ]);
    ...
    return (
        <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
            ...
            <div className="flex items-center justify-between">
                <h1 className="hidden md:block text-lg font-semibold">Roditelji</h1>
                <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                    <TabelaPretrage />
                    <div className="flex items-center gap-4 self-end">
                        <button className="w-8 h-8 flex items-center justify-center founded-full">
                        ...
                        {role === 'admin' && (
                            <FormeContainer table="roditelj" type="create" />
                        )}
                    </div>
                </div>
            </div>
